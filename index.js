...
async function init() {
	...
	const rgbeLoader = new RGBELoader().setPath('public/assets/')
	const gltfLoader = new GLTFLoader().setPath('public/assets/')
	gltfLoader.setDRACOLoader(new DRACOLoader().setDecoderPath('jsm/libs/draco/obj/'))

	// load the environment map
	const [env, model] = await Promise.all([rgbeLoader.loadAsync('blouberg_sunrise_2_1k.hdr'), gltfLoader.loadAsync('bowl.glb')])
	...
	bowlModel = model.scene.children[0]
	bowlModel.receiveShadow = true
	bowlModel.castShadow = true
	...
	const valuesChanger = function() {
		...
		bowlsEnabled = effectController.bowlsEnabled
		let i = NUM_BOWLS
		while (i--) {
			if (bowls[ i ]) bowls[ i ].visible = bowlsEnabled
		}
	}
	...
	gui.add(effectController, 'bowlsEnabled').onChange(valuesChanger)
	...
	gui.add(buttonSmooth, buttonSmooth, 'smoothWater')
	initWater()
	createBowls()
	valuesChanger()
...
function createBowls() {
	for (let i = 0; i < NUM_BOWLS; i++) {
		let sphere = bowlModel
		if (i < NUM_BOWLS - 1) {
			sphere = bowlModel.clone()
		}
